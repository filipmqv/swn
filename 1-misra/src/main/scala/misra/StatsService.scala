...
import akka.actor.{Actor, ActorRef, ActorSelection, Props, ReceiveTimeout, RootActorPath}
...
//  val workerRouter = context.actorOf(FromConfig.props(Props[StatsWorker]), name = "workerRouter")
  ...
  def receive = {
    case NextNode(address) =>
      nextNode = address
      client = sender()
      println("got nextNode")
      nextNode ! Ping(666)
    case Ping(value) =>
      println("got ping from " + sender + " " + value)
